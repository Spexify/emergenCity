extends CanvasLayer
class_name EMC_DialogueGUI

@onready var response_template: RichTextLabel = $Balloon/Margin/HBox/VBox/ResponseTemplate #@export 
...
var dialogue_line: DialogueLine:
	set(next_dialogue_line):
		...
		var sprite_texture: CompressedTexture2D = \
			load("res://res/sprites/characters/portrait_" + dialogue_line.character.to_lower() + ".png")
...
func setup(p_dialogue_pitches: Dictionary) -> void:
	_dialogue_pitches = p_dialogue_pitches
	pass
...
func _on_dialogue_label_spoke(letter: String, letter_index: int, speed: float) -> void:
	if not letter in [" ", "."]:
		...
		if letter_index % actual_speed == 0:
			talk_sound.play()
			#var pitch: float = DIALOGUE_PITCHES.get(dialogue_line.character, 1)
			#Not an if-statement, this uses the ternary-if expression:
			var pitch: float = _dialogue_pitches[dialogue_line.character] \
				 if _dialogue_pitches.has(dialogue_line.character) else 1.0 
			if dialogue_line.get_tag_value("mood") == "happy":
				pitch += 0.1
			elif dialogue_line.get_tag_value("mood") == "sad":
				pitch -= 0.1
				
			talk_sound.pitch_scale = randf_range(pitch - 0.05, pitch + 0.05)
