...
@export var warn_texture : Image
@onready var item_list : EMC_Item_List = $ItemList
...
func start() -> void:
	...
	item_list.add_item("Der Strom ist " + OverworldStatesMngr.get_electricity_state_descr(), warn_texture, Rect2(x, y, 64, 64))
	
	x = OverworldStatesMngr.get_water_state() * 64
	y = OverworldStatesMngr.name_to_state["WaterState"][1] * 64
	item_list.add_item("Das Wasser ist " + OverworldStatesMngr.get_water_state_descr(), warn_texture, Rect2(x, y, 64, 64))
	...
	" + OverworldStatesMngr.get_isolation_state_descr(), warn_texture, Rect2(x, y, 64, 64))
	
	x = OverworldStatesMngr.get_food_contamination_state() * 64
	y = OverworldStatesMngr.name_to_state["FoodContaminationState"][1] * 64
	item_list.add_item("Der Strom ist " + OverworldStatesMngr.get_food_contamination_state_descr(), warn_texture, Rect2(x, y, 64, 64))	
	...
	momentan " + OverworldStatesMngr.get_mobile_net_state_descr(), warn_texture, Rect2(x, y, 64, 64))
