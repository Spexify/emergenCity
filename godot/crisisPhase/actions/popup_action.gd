...
func _init(p_action_ID: int, p_action_name : String, p_constraints_prior : Dictionary,
 p_consequences : Dictionary, 
			p_description : String, p_performance_coin_value : int, p_pop_up_text : String) -> void:
	super(p_action_ID, p_action_name, p_constraints_prior, p_consequences, "", p_description, p_performance_coin_value)
...
static func from_dict(data : Dictionary) -> EMC_PopUpAction:
	...
	return EMC_PopUpAction.new(_action_id, _action_name, _constraints, _consequences, _description, _e_coin, _pop_up_text)
