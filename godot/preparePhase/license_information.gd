...
@onready var rich_text_label : RichTextLabel = $MarginContainer2/VBoxContainer/RichTextLabel
@onready var link_button : LinkButton = $MarginContainer2/VBoxContainer/LinkButton

func open(p_name : String) -> void:
	show()
	...
	match p_name:
		"open":
			...
			link_button.set_text("OpenDyslexic")
			link_button.set_uri("https://opendyslexic.org/")
			link_button.set_underline_mode(LinkButton.UNDERLINE_MODE_ALWAYS)
		"cmu":
			rich_text_label.set_text(
			...
			FROM, OUT OF THE USE OR INABILITY TO USE THE FONT SOFTWARE OR FROM
			OTHER DEALINGS IN THE FONT SOFTWARE.')
			link_button.set_text("CMU Typewriter")
			link_button.set_uri("")
			link_button.set_underline_mode(LinkButton.UNDERLINE_MODE_NEVER)
