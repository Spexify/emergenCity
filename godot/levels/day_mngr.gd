...
func on_interacted_with_furniture(action_id : int):
	...
	for constrain_key in current_action.get_constrains_prior().keys():
		if not Callable(self, constrain_key).call(current_action.get_constrains_prior()[constrain_key]):
			rejected_constrains.append(constrain_key)
	
	if rejected_constrains.size() >= 1:
		current_action.set_constrains_rejected(rejected_constrains)
